from sympy import isprime, nextprime

"""
But     : produire (p, g) avec p premier (a <= p < b) et g d'ordre p-1 modulo p.
          Il faut également fournir la factorisation de p-1.
          p-1 doit être un multiple de q.
"""

def find_p_and_factorization(a, b, q, n = 50):
    """
    Recherche de p et de la factorisation de (p-1)//q
    tel que (a <= p < b) et (p - 1) soit un multiple de q
    paramètres: a -> borne inférieure
                b -> borne supérieure
                q -> facteur
                n -> nombre de facteurs premiers de (p-1)//q
    """
    factors = [2] # Liste des facteurs premiers de (p-1)//q
    p_k = 2       # facteur premier courant
    Q = 2*q       # produit des facteurs premiers (trouvé pour l'instant)

    # Générer d'abord de manière arbitraire n facteurs premiers de (p-1)//q
    for i in range(n):
        p_k = nextprime(p_k)
        factors.append(p_k)
        Q *= p_k
    
    lower_bound = a // Q          # borne inférieure divisée par les facteurs premiers trouvés jusqu'à présent
    p_k = nextprime(lower_bound)
    p_1 = Q * p_k                 # p_1 = p-1 = 2*q*...+ 1 = Q*p_k + 1
    i = 0

    # Tant que p_1 est dans l'intervalle [a-1, b-1), on cherche le prochain facteur premier qui remplit les conditions de p
    while (a-1 <= p_1 < b-1):
        i+=1
        print(f"iteration {i}:")
        if (p_1 % q == 0 and isprime(p_1 + 1)):
            factors.append(p_k)
            return p_1 + 1, factors
        lower_bound = p_k
        p_k = nextprime(lower_bound)
        p_1 = Q * p_k




def find_generator(p, factors):
    """
    Recherche d'un générateur g
    paramètres: p -> nombre premier
                q -> facteur
                r -> tableau de facteurs premiers de (p-1)//q
    """
    phi = p - 1
    for g in range(2, p):
        if all(pow(g, phi // factor, p) != 1 for factor in factors):
            return g
    return None

if __name__ == "__main__":
    # données
    a_hex = ""
    q_hex = "f11d92ca48f6231885ef109e402dfbf2c5655e02653b6efd0aea6852b53e9afd9bc3bb52727b319c3a1580c36f0dc03b"
    a = int(a_hex, 16)
    b = a + 2**1950
    q = int(q_hex, 16)

    # Recherche de p et de la factorisation de (p-1)//q
    p, factors = find_p_and_factorization(a, b, q)
    if p is not None:
        print(f"\nTrouvé factorisation de (p-1)/q:\n{factors}")
    else:
        print("Aucun p valide trouvé.")

    # Recherche d'un générateur g
    g = find_generator(p, factors)
    if g is not None:
        print(f"\nTrouvé g: \n{g}")
    else:
        print("Aucun générateur valide trouvé.")
